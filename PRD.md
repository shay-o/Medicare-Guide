...
- The should be a way to see results for any test run including individual and aggregate scores.
...
- Result scoring: Each individual response will be scored from 1 to 10. There will also be aggregations of these individual scores. There will be an average across the LLM for all questions, an average across the question for all LLMs, and an average across all questions and models for a given test run.

### Scoring
- For this version scoring will be done by comparing the similarity between the response and the correct answer. An exact match will be a 10. The greater the semantic difference the lower the score.
- The score will be recorded for each response to each question. Aggregate scores will be calculated from the scores for the individual responses.
